...
变量是程序的**基本组成单位**，变量相当于**内存中一个数据存储空间的表示**。变量不是一个地址，而是收尾地址所包含的内容，**通过变量名可以访问到变量(值)**。变量应该有名字，并在内存中占据一定的存储单元；变量名和变量值有不同的含义；变量名实为一个符号地址。
...
编译器是对代码进行编译，转换为包含在目标文件中的机器语言版本(字节码)，生成扩展名为.o/.obj的目标文件；解释器是能够执行用其他计算机语言编写的程序的系统软件，它是一种翻译程序。
